  ...
  const { data: runs } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("id, period_start, period_end, status, total_litres, total_amount, farmer_count, created_at")
        .eq("mcc_id", mcc!.mccId)
  ...
  return (
    <AppShell nav={FINANCE_NAV}>
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        />
        <StatCard label="Farmers in period" value={preview.farmers} icon={<Users className="h-4 w-4" />} />
        <StatCard
          label="Outstanding in run"
          value={formatCurrency(outstanding)}
          icon={<Wallet className="h-4 w-4" />}
        />
        <StatCard label="Settlement runs" value={runs?.length ?? 0} icon={<BookOpenCheck className="h-4 w-4" />} />
      </div>

      <div className="surface-card mt-4 grid gap-3 p-5 sm:grid-cols-4">
        <div>
          <Label htmlFor="from">Period from</Label>
          <Input id="from" type="date" className="mt-1 h-11" value={from} onChange={(e) => setFrom(e.target.value)} />
        </div>
        <div>
          <Label htmlFor="to">Period to</Label>
          <Input id="to" type="date" className="mt-1 h-11" value={to} onChange={(e) => setTo(e.target.value)} />
        </div>
        <div>
        ...
        <div className="flex items-end">
          <Button className="h-11 w-full" onClick={() => createRun.mutate()} disabled={createRun.isPending}>
